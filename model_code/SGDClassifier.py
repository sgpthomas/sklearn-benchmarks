...
def get_pipeline_parameters():
    ...
    all_param_combinations = itertools.product(loss_values,
                                               penalty_values,
                                               alpha_values,
                                               learning_rate_values,
                                               fit_intercept_values,
                                               l1_ratio_values,
                                               eta0_values,
                                               power_t_values,
                                               random_state)
    pipeline_parameters = [{'loss': loss,
                            'penalty': penalty,
                            'alpha': alpha,
        ...
        if not (penalty != 'elasticnet' and l1_ratio != 0.15)
                           and not (learning_rate not in ['constant', 'invscaling'] and eta0 != 0.0)
                           and not (learning_rate != 'invscaling' and power_t != 0.5)]
...
def run(dataset, params, resultdir=".", use_params=True):
    pipeline_parameters = {}
    if use_params:
        pipeline_parameters[SGDClassifier] = params
